...
import { ArrowUpRight, ArrowRight, User, ChevronDown, Bell } from 'lucide-react';

export default function TopbarActions({ isMobile = false }: { isMobile?: boolean }) {
      ...
      {status !== 'loading' && !user && (
        <div className="flex items-center gap-6">
          <Link
            ...
            className="inline-flex h-[50px] items-center justify-center gap-2 rounded-lg border border-fv-border-strong bg-white px-6 py-2.5 font-[var(--font-inter)] text-lg leading-[26px] font-medium text-fv-ink hover:bg-fv-parchment/60 transition-colors whitespace-nowrap"
          >
            Log in
          ...
          <Link
            ...
            className="inline-flex h-[50px] items-center gap-3 rounded-xl bg-fv-ink pl-4 pr-1 font-[var(--font-inter)] text-lg leading-[26px] font-medium text-white hover:bg-fv-ink/90 transition-colors whitespace-nowrap"
          >
            <span>Get Started</span>
            <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white text-fv-ink">
